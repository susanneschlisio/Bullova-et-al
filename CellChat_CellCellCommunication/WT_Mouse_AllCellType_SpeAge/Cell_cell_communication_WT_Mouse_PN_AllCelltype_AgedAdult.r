set.seed(1234)
setwd("./WT_Mouse_AllCellType_SpeAge")
...
SelectedAge <- "AgedAdult"
query_subset <- subset(query, subset = wenyu_names %in% c("AdultAG"))
...
saveRDS(cellChat, paste0("WT_Mouse_",SelectedAge,"_AllCelltype_CellChat.RDS"))

# Compute network centrality scores
cellChat <- netAnalysis_computeCentrality(cellChat, slot.name = "netP")

saveRDS(cellChat, paste0("WT_Mouse_",SelectedAge,"_AllCelltype_CCC.rds"))
rm(query, query_subset)
save.image(paste0("WT_Mouse_",SelectedAge,"_AllCelltype_CCC.RData"))
